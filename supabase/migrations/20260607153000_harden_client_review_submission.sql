-- Harden client-side review submission against production schema drift.
-- The live app sends feedback_text; older production review tables may not have
-- that column in PostgREST's schema cache yet.
...
DECLARE
  source_column text;
BEGIN
  FOREACH source_column IN ARRAY ARRAY['feedback', 'comment', 'review_text', 'comment_text']
  LOOP
    IF EXISTS (
      SELECT 1
      ...
      EXECUTE format(
        'UPDATE public.reviews SET feedback_text = COALESCE(feedback_text, %I::text) WHERE feedback_text IS NULL',
        source_column
      );
      EXIT;
    END IF;
...
BEGIN
  ...
  SET
    status = 'completed',
    confirmed_by_client = COALESCE(confirmed_by_client, v_actor),
